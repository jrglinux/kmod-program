 ...
 * than call the original function (sys_getpid), is because somebody else
 * might have replaced the system call before us. Note that this is not
 * 100% safe, because if another module replaced sys_getpid before us,
...
/* The function we will replace sys_getpid (the function called when you
 * call the getpid system call) with.
 *
 ...
 */
static asmlinkage int our_sys_getpid(void)
{
    pr_info("our_sys_getpid, current pid:%d\n", current->pid);
    /* Call the original sys_getpid - otherwise, we lose the ability to
     * get process's pid number.
     */
...
static int __init syscall_start(void)
{
    ...
    /* keep track of the original getpid function */
    original_call = (void *)sys_call_table[__NR_getpid];

    /* use our getpid function instead */
    sys_call_table[__NR_getpid] = (unsigned long *)our_sys_getpid;
    
    enable_write_protection();
    
    pr_info("replace syscall getpid with our_sys_getpid\n");
...
static void __exit syscall_end(void)
{
    ...
    if (sys_call_table[__NR_getpid] != (unsigned long *)our_sys_getpid) {
        pr_alert("Somebody else also played with the ");
        pr_alert("getpid system call\n");
        pr_alert("The system may be left in ");
        pr_alert("an unstable state.\n");
    }
    
    disable_write_protection();
    sys_call_table[__NR_getpid] = (unsigned long *)original_call;
    enable_write_protection();
